...
function doPost(e) {
  ...
  UrlFetchApp.fetch(
    config.REPLY_URL,
    createReplyRequest(replyToken, replyMessage)
  );
...
const handleUserMessage = (e) => {
  ...
  let replyMessage = convertUserMessageToReplyMessage(
    messageValidation(lastEventType, lastMessage, userMessage)
  );
...
const messageValidation = (lastEventType, lastMessage, userMessage) => {
  ...
  if (messageLength > 1) {
    ...
    if (lastEventType === 'postback' && lastMessage === 'add') {
      ...
      message = commands.includes(messageList[0])
        ? messageList[0]
        : 'notExistCommand';
    }
  }
 ...
 * @param          replyMessage - message情報 複数なら配列、一つならオブジェクト
...
const createReplyRequest = (replyToken, replyMessage) => {
  ...
  if (Array.isArray(replyMessage)) {
    for (let i = 0; i < replyMessage.length; i++) {
      replyObj.payload.messages.push(replyMessage[i]);
    }
  } else {
    replyObj.payload.messages.push(replyMessage);
  }

  replyObj.payload = JSON.stringify(replyObj.payload);
